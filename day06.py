...
class Object:
    ...
    def add_orbit(self, new_object):
        self.orbits.append(new_object)
        new_object.orbiting = self

    def total_orbits(self):
        return self.search_orbits() + sum(i.total_orbits() for i in self.orbits)

    def search_orbits(self):
        total = 0
        for i in self.orbits:
            total += 1 + i.search_orbits()
        return total

    def search_orbits_item(self, item):
        path = [self]
        if item in self.orbits:
            return path
        for i in self.orbits:
            rest = i.search_orbits_item(item)
            if rest is not None:
                path += rest
                return path
