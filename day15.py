...
def explore_map():
    ...
    if map[location[0] + 1, location[1]] == -1:
        direction = 4
        return 4
    if map[location[0] - 1, location[1]] == -1 and location[0] >= -50:
        direction = 3
        return 3
    ...
    return [[1, 2, 1], [3, 3, 3], [4, 4, 4]][dx][dy]


map[0, 0] = 1
computer = Computer(data, in_function=explore_map, out_function=fill_in_map)
try:
    computer.run_until_complete()
except StopIteration:
    pass
print("filled map")
map_search = defaultdict(lambda: 99999)


def search_map(x, y, n):
    if map[x, y] == 2:
        raise StopIteration
    if map_search[x - 1, y] == 99999 and map[x - 1, y] > 0:
        map_search[x - 1, y] = n + 1
        search_map(x - 1, y, n + 1)
    if map_search[x + 1, y] == 99999 and map[x + 1, y] > 0:
        map_search[x + 1, y] = n + 1
        search_map(x + 1, y, n + 1)
    if map_search[x, y - 1] == 99999 and map[x, y - 1] > 0:
        map_search[x, y - 1] = n + 1
        search_map(x, y - 1, n + 1)
    if map_search[x, y + 1] == 99999 and map[x, y + 1] > 0:
        map_search[x, y + 1] = n + 1
        search_map(x, y + 1, n + 1)
